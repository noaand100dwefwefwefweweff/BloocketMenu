...
`);
    ...
    GUI.style.background = 'rgba(173, 216, 230, 0.95)';
    ...
    header.innerHTML = `Cum Balls <span style="font-size: 0.75rem">(‿ˠ‿)</span>`;

    let loop;

    let close = document.createElement('button');
    header.appendChild(close);
    close.style.background = 'rgba(173, 216, 230, 0.95)';
    ...
    minimize.style.background = 'rgba(173, 216, 230, 0.95)';
    ...
    footer.innerHTML = (`<span>Cheats by Noah<br>secert 🤫 <a style="color: red" href="https://www.youtube.com/watch?v=3qi4OaOKyZI">CLICK ME!</a></span>`);
            ...
            try {
                ...
                if (stateNode.state.stage === 'prize') {
                    ...
                    if (chestContainer && chestContainer.children.length === 3 && !chestContainer.querySelector('.picked-by-script')) {
                        ...
                        // Prefer triple, then double, then any gold
                        for (let i = 0; i < choices.length; i++) {
                            let text = choices[i].text.toLowerCase();
                            if (text.includes("triple gold")) { pickIndex = i; break; }
                            if (text.includes("double gold")) { pickIndex = i; }
                        }

                        if (pickIndex === -1) {
                            pickIndex = choices.findIndex(c => c.text.toLowerCase().includes("gold"));
                        }

                        // If no gold chest, look for swap and use leaderboard logic
                        if (pickIndex === -1) {
                            ...
                            if (swapIndex !== -1) {
                                ...
                                if (topPlayer && topPlayer !== reactHandler().props.client.name) {
    ...
    try {
        Array.from(document.querySelector('div[class*="answersHolder"').children).forEach(x => {
            ...
            let existingDot = x.querySelector('.answer-dot');
            if (existingDot) existingDot.remove();

            // If it's a correct answer, add the dot
            if (
                reactHandler().memoizedState.question.correctAnswers.includes(x.innerText) ||
                reactHandler().memoizedProps.client.question.correctAnswers.includes(x.innerText)
            ) {
                ...
                dot.style = `
                    ...
                    z-index: 999;
                `;
                x.style.position = 'relative'; // Ensure the container is position-relative
                x.appendChild(dot);
            }
        });
    } catch { }
...
function toggleAutoAnswer(e) {
    if (e.code === 'KeyF') {
        autoAnswer = !autoAnswer;
    }
}
addEventListener('keydown', toggleAutoAnswer);

function shutdownMenu(e) {
    if (e.code === 'KeyP') {
        GUI.remove();
        clearInterval(loop);
        removeEventListener('keydown', toggleAutoAnswer);
        removeEventListener('keypress', toggleHidden);
        removeEventListener('keydown', shutdownMenu);
    }
}
...
addEventListener('keydown', toggleHighlight);
addEventListener('keydown', shutdownMenu);
})()
